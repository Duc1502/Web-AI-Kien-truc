...
// Đọc thông tin tài khoản nhận tiền (số TK, mã ngân hàng, chủ TK) từ settings.sepay_bank_info.
async function getSepayBankInfo(): Promise<{ account_number: string; bank_code: string; account_holder: string } | null> {
  if (!supabaseAdmin) return null;
  const { data } = await supabaseAdmin.from("settings").select("value").eq("key", "sepay_bank_info").single();
  const info = (data?.value as any) || null;
  if (!info || !info.account_number || !info.bank_code) return null; // chưa cấu hình
  return info;
}

// Tạo URL ảnh QR VietQR của SePay (không cần API key). Nội dung CK = mã đơn để webhook tự khớp.
function buildSepayQrUrl(
  bank: { account_number: string; bank_code: string },
  amountVnd: number,
...
export const handleCreateOrder = async (req: any, res: any): Promise<any> => {
  try {
    ...
    // Xác thực người dùng qua access token thay vì tin userId từ body (chống tạo đơn hộ người khác).
    const authHeader = req.headers.authorization;
    const accessToken = authHeader?.startsWith("Bearer ") ? authHeader.slice(7) : null;
    if (!accessToken) {
      return res.status(401).json({ error: "Vui lòng đăng nhập để mua credit." });
    }
    ...
    const { planId, fullName, phone } = req.body;
    if (!planId || !fullName || !phone) {
    ...
    // Mã đơn = nội dung chuyển khoản duy nhất. Chỉ gồm A-Z0-9 để ngân hàng giữ nguyên khi CK.
    const referenceCode =
    ...
    const qrUrl = bank ? buildSepayQrUrl(bank, plan.price, referenceCode) : null;

    return res.json({
      transactionId: data.id,
      referenceCode,
      amountVnd: plan.price,
      qrUrl, // null nếu admin chưa cấu hình tài khoản nhận tiền
      bankInfo: bank
        ? { accountNumber: bank.account_number, bankCode: bank.bank_code, accountHolder: bank.account_holder }
        : null,
    });
  } catch (error: any) {
...
// API: Webhook SePay — SePay gọi khi có tiền vào tài khoản ngân hàng đã liên kết. Tự khớp mã đơn
// (nội dung CK) → cộng credit. Bảo vệ bằng secret ở header Authorization: Apikey <SEPAY_WEBHOOK_API_KEY>.
export const handleSepayWebhook = async (req: any, res: any): Promise<any> => {
  try {
    if (!supabaseAdmin) {
      return res.status(500).json({ success: false, error: "Server chưa cấu hình Supabase." });
    }

    // Xác thực secret (fail-closed: chưa đặt env → từ chối để tránh bị giả mạo cộng credit).
    const expectedKey = process.env.SEPAY_WEBHOOK_API_KEY;
    if (!expectedKey) {
      console.error("SEPAY_WEBHOOK_API_KEY chưa được đặt trên server.");
      return res.status(500).json({ success: false, error: "Webhook chưa được cấu hình." });
    }
    const provided = String(req.headers.authorization || "").replace(/^Apikey\s+/i, "").trim();
    if (provided !== expectedKey) {
      return res.status(401).json({ success: false, error: "Sai khóa xác thực webhook." });
    }
    ...
    const normContent = String(payload.content || "").toUpperCase().replace(/[^A-Z0-9]/g, "");

    // Tìm đơn đang chờ có mã (reference_code) nằm trong nội dung chuyển khoản.
    const { data: pendings } = await supabaseAdmin
      .from("transactions")
      .select("id, user_id, amount_vnd, package_id, reference_code")
      .eq("status", "pending")
      .eq("method", "vietqr")
      .order("created_at", { ascending: false })
      .limit(300);

    const matched = (pendings ?? []).find((t) => {
      const code = String(t.reference_code || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
      return code.length >= 4 && normContent.includes(code);
    });
    ...
    if (matched) {
      ...
      // Lưu payload gốc để đối soát/debug, rồi cộng credit nguyên tử + idempotent.
      await supabaseAdmin.from("transactions").update({ raw_payload: payload }).eq("id", matched.id);
      const { data: newBalance, error: rpcErr } = await supabaseAdmin.rpc("confirm_transaction", {
      ...
      return res.json({ success: true, matched: true, transactionId: matched.id, creditsAdded: newBalance === null ? 0 : credits });
    }

    // Không khớp đơn nào → lưu 'unmatched' để admin đối soát thủ công (chống mất tiền của khách).
    await supabaseAdmin.from("transactions").insert({
      amount_vnd: amount,
      method: "vietqr",
      status: "unmatched",
      reference_code: String(payload.content || "").slice(0, 200) || null,
      raw_payload: payload,
    });

    return res.json({ success: true, matched: false, message: "Đã lưu giao dịch chưa khớp để đối soát." });
  } catch (error: any) {
    console.error("Error handling SePay webhook:", error);
    return res.status(500).json({ success: false, error: error.message || "Lỗi xử lý webhook." });
  }
};

// API: Trang checkout hỏi trạng thái đơn (poll) để hiện "thành công" khi webhook đã cộng credit.
export const handleTransactionStatus = async (req: any, res: any): Promise<any> => {
  try {
    if (!supabaseAdmin) {
    ...
    const authHeader = req.headers.authorization;
    const accessToken = authHeader?.startsWith("Bearer ") ? authHeader.slice(7) : null;
    if (!accessToken) return res.status(401).json({ error: "Vui lòng đăng nhập." });

    const { data: authData, error: authError } = await supabaseAdmin.auth.getUser(accessToken);
    if (authError || !authData?.user) return res.status(401).json({ error: "Phiên đăng nhập hết hạn." });
    const userId = authData.user.id;

    const transactionId = req.body?.transactionId || req.query?.transactionId;
    if (!transactionId) return res.status(400).json({ error: "Thiếu transactionId." });
    ...
    // Trả kèm số dư hiện tại để client cập nhật ngay khi thành công.
    ...
    return res.json({ status: tx.status, creditsRemaining: profile?.credits_balance ?? null });
  } catch (error: any) {
    console.error("Error checking transaction status:", error);
    return res.status(500).json({ error: error.message || "Lỗi kiểm tra trạng thái." });
  }
};
...
export const handleGenerate = async (req: any, res: any): Promise<any> => {
  ...
  try {
    ...
    // Kiểm tra số dư TRƯỚC khi render nhưng CHƯA trừ — chỉ trừ SAU khi render thành công (bên dưới).
    // Nhờ vậy render lỗi/timeout giữa chừng (vd 4K vượt giới hạn thời gian của serverless) sẽ KHÔNG
    // làm mất credit của người dùng.
    const { data: profileRow } = await supabaseAdmin
      .from("profiles")
      .select("credits_balance, is_locked")
      .eq("id", userId)
      .single();
    if (!profileRow) {
      return res.status(500).json({ error: "Không đọc được số dư credit của tài khoản." });
    }
    if (profileRow.is_locked) {
      return res.status(403).json({ error: "Tài khoản đang bị khoá." });
    }
    if (profileRow.credits_balance < cost) {
      return res.status(402).json({
        error: "Không đủ credit để thực hiện render này. Vui lòng mua thêm credit tại trang Bảng Giá.",
      });
    }

    deductedUserId = userId; // đánh dấu đã xác thực để ghi lịch sử generations (kể cả khi lỗi)
...
Không được tự ý thiết kế lại, sáng tạo thêm hoặc chỉnh sửa kiến trúc dưới bất kỳ hình thức nào. Mọi thay đổi chỉ được phép nhằm tăng tính chân thực của hình ảnh, tuyệt đối không làm thay đổi ý đồ thiết kế ban đầu.${englishContext}`;
    ...
    if (supabaseAdmin && deductedUserId) {
      // Render THÀNH CÔNG → giờ mới trừ credit (atomic). Nếu vì race (số dư vừa bị tiêu hết bởi
      // request khác) mà không trừ được thì vẫn trả ảnh — hiếm vì đã pre-check số dư ở trên.
      const { data: nb } = await supabaseAdmin.rpc("deduct_credits", { p_user_id: deductedUserId, p_amount: cost });
      newBalance = nb ?? null;
      deductedAmount = cost;
    ...
    // Credit chỉ bị trừ SAU khi render thành công → lỗi ở đây nghĩa là CHƯA trừ, không cần hoàn.
    // Chỉ ghi log lỗi để Admin Dashboard theo dõi tỉ lệ lỗi.
    if (supabaseAdmin && deductedUserId) {
      await supabaseAdmin.from("generations").insert({
        id: generationId,
        user_id: deductedUserId,
...
app.post("/api/checkout/status", handleTransactionStatus);
